...
object SparkActiveUserMonitor {
  val LOGGER = Logger.getLogger(classOf[SparkActiveUserMonitor].getName)
}

class SparkActiveUserMonitor() extends DataMonitor {
  ...
  override def run(xcurrentDate: String, threshold: String, orgId: String): Unit = {
    ...
    SparkActiveUserMonitor.LOGGER.info("Entering program. currentDate: " + myDate)
    ...
    val dsFilteredByOrgs = input.where("orgid in (" + whereOrgIdClause + ") and relation_name='activeUser' and month = '" + month + "' and period = 'daily' and time_stamp = '" + myDate + "' ")

    val sumPerOrg = getSumPerOrg(dsFilteredByOrgs, myDate)
    val sparkactiveUserMsges = createMessages(sumPerOrg, true)
    writeToKafka(sparkactiveUserMsges, false)
  }
  ...
  private def createMessages(dataset: Dataset[Row], alert: Boolean) = {
    val message = dataset.selectExpr("'crs' as component",
      ...
      "struct('Spark' as pipeLine, " +
        "struct(" + "orgid, " + "onetoonecount, " + "spacecount, " + 
